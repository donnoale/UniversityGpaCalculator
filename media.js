$(function(){
		...
		$(document).on("keydown", "input", function(e) {
			var focusNextRow = function($curtd){
				...
				if($curRow.next().length == 0) $("#piu").click();
				if(esameOn) $curRow.next().find(".esame").focus().select();
				else $curRow.next().find(".voto").focus().select();
			}
			
			if (e.keyCode != 13) return;
			if($(this).hasClass("esame")) $(this).closest("tr").find(".voto").focus().select();
			if($(this).hasClass("voto")) {
				if(!creditiOn) focusNextRow($(this));
		...
		$("#piu").click(function() {
			$row = $('<tr><td><input type="button" class="meno" value="-"/></td><td><input type="text" class="esame"/></td><td><input type="text" class="voto" /></td><td><input type="text" class="crediti"/></td></tr>');
			
			if(!esameOn) $row.find("td:nth-child(2)").addClass("hidden");
			if(!creditiOn) $row.find("td:nth-child(4)").addClass("hidden");
		...
		$("#calc").click(function() {
			...
			if(creditiOn) $("#laurea").html(Math.round(mediaP*11/3)+' ('+(mediaP*11/3).toFixed(4)+')');
			else $("#laurea").html(Math.round(mediaA*11/3)+' ('+(mediaA*11/3).toFixed(4)+')');
			
			$("#nEsami").html(numEsami);
			
			if(!creditiOn) $("#totCred, #mediaP").html("ignorato");
	...
	var verificaVoto = function($input) {
		var vot = $input.val();
		var regexp = /[\s]+$/;
		if(!isNum(vot) || vot<18 || vot>31) {
			$input.addClass("err");
			return false;
		}
		else $input.removeClass("err warn");
